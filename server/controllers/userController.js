...
exports.find = (req, res) => {
  ...
  connection.query('SELECT * FROM student WHERE id LIKE ? OR name LIKE ? OR gender LIKE ? OR department LIKE ?', 
...
exports.finds = (req, res) => {
  ...
  connection.query('SELECT DISTINCT id FROM result WHERE id LIKE ?', ['%' + searchTerm + '%'], 
    (err, rows) => {
      if(rows.length === 0 ){
...
exports.update = (req, res) => {
  const { id, name, batch, gender, department, phone, email } = req.body;
  connection.query('UPDATE student SET ? WHERE id = ?', 
...
exports.delete = (req, res) => {
  connection.query('DELETE FROM student WHERE id = ?', [req.params.id], (err, rows) => {
    if (!err) {
      connection.query('UPDATE student SET status = ? WHERE id = ?', ['removed', req.params.id], (updateErr, updateRows) => {
        if (!updateErr) {
          let removedUser = encodeURIComponent('Student successfully removed.');
          return res.redirect('/?removed=' + removedUser);
        } else {
